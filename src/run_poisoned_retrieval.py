...
OUTPUT_PATH = (
    PROJECT_ROOT / "results" / "poisoned_retrieval.csv"
)
...
def add_poison_documents(clean_corpus, poison_documents):
    poisoned_corpus = list(clean_corpus)

    for poison in poison_documents:
        poisoned_corpus.append(
            {
                # Postojeće BM25 funkcije koriste naziv "pmid"
                # kao ID dokumenta. Za poison koristimo poison_id.
                "pmid": poison["poison_id"],
                "context": poison["attack_text"],
                "tokens": tokenize(poison["attack_text"]),
            }
        )
...
def find_rank(ranked_documents, document_id):
    return next(
        (
            rank
...
def main():
    ...
    for index, question in enumerate(questions, start=1):
        ...
        result = {
            ...
            "number_of_poisons_in_top_5": len(
                poisons_in_top_5
            ),
    ...
    print()
    print(f"Target poison Retrieval ASR@1: {poison_at_1:.2%}")
